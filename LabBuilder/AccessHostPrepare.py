...
def installComponents(clusterDictionary):
    ...
    while not connected:
        try:
            ...
            (stdin, stdout, stderr) = ssh.exec_command("sudo echo 'source /opt/rh/python27/enable' >> /home/instructor/.bashrc;chown -R instructor: /home/instructor")
            print stderr.readlines()
            print stdout.readlines()
            (stdin, stdout, stderr) = ssh.exec_command("sudo echo 'source /opt/rh/python27/enable' >> ~/.bashrc")
            print stderr.readlines()
            print stdout.readlines()
            (stdin, stdout, stderr) = ssh.exec_command("sudo pip install pip -U;pip install setuptools -U; pip install numpy -U")
            print stderr.readlines()
            print stdout.readlines()
            ...
            print stderr.readlines()
            print stdout.readlines()
            (stdin, stdout, stderr) = ssh.exec_command("sudo pip install nltk -U ;python -m nltk.downloader all")
            print stderr.readlines()
            print stdout.readlines()
            (stdin, stdout, stderr) = ssh.exec_command("sudo pip install gensim -U >> /tmp/gensim.out")
            print stderr.readlines()
            print stdout.readlines()
